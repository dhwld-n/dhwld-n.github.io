...
const PROJECTS = {
  ...
  tinker: {
    icon: "🍀",
    title: "사주 · 운세 웹앱 Tinker",
    meta: [
      ["개발 기간", "2026.08 ~ 진행 중 (개인 프로젝트)"],
      ["인원 구성", "1인 개발 (기획 · 구현 · 디자인 · 배포)"],
      ["본인 역할", "명리 계산 로직 이식, LLM 스트리밍 파이프라인, 디자인 시스템, 배포까지 전 과정 단독"],
      ["형태", "정적 SPA + 서버리스 함수 · Vercel 배포"],
    ],
    overview:
      "사주 · 자미두수 · 기문둔갑 · 궁합 · 타로 · 오늘의 운세 등 '심심풀이' 도구를 한 페이지에 모은 " +
      "웹앱입니다. 명리 계산은 전부 브라우저에서 돌아가고(파이썬 원본을 JS로 포팅), 풀이 문장만 " +
      "Gemini가 스트리밍으로 생성합니다. 친구들이 링크 하나로 바로 쓸 수 있게 만들었고 도구를 계속 늘려 가는 중입니다.",
    work: [
      "명리 엔진 브라우저 이식 — saju.py · jamidusu.py의 일주 · 명궁 · 오행국 · 오행 분포 계산을 calc.js로 포팅하고 파이썬 원본과 동일 출력을 검증. 음력 변환은 경량 라이브러리를 번들.",
      "LLM 스트리밍 — Vercel 서버리스 함수가 Gemini SSE를 중계하고, 클라이언트는 읽기 속도에 맞춰 페이스를 조절해 노출. 같은 입력엔 같은 결과가 나오도록 localStorage · KV 캐시를 둠.",
      "결정론적 개인화 — 행운의 색 · 숫자 · 물건, 수호 동물, 닮은 공룡 등은 사주 팔자를 시드로 해시해 고정 값으로 뽑음. 같은 사람은 늘 같은 결과, 다른 친구는 다른 결과.",
      "오행 시그니처 디자인 — 명식 표가 실제로 담고 있는 오행(목화토금수)에 따라 물드는 오방색 액센트 시스템. 레퍼런스 대시보드와 frontend-design 방법론으로 구조 · 타이포를 설계.",
      "이미지 분석 — 얼굴 사진 업로드 또는 웹캠 스캔을 Gemini 비전으로 분석해 피부 상태 · 루틴 추천, '나와 얼굴이 닮은 공룡' 같은 기능 구현.",
    ],
    stack: [
      "JavaScript (Vanilla)",
      "Vercel Serverless",
